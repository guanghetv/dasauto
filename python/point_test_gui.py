...
#         input_text = Text()
        ...
        input_sb.config(command=self.input_text.yview)
        self.input_text.config(yscrollcommand=input_sb.set)
        input_sb.pack(side=RIGHT, fill=Y)
        self.input_text.pack(expand=YES, fill=BOTH)
        
        console_frame = Frame(master)
        console_frame.pack()
        start_btn = Button(console_frame,text="开始",command=self.start_test)
#         stop_btn = Button(console_frame,text="退出",command=self.stop_test)
...
#         stop_btn.pack(side=LEFT)
        ...
        output_sb.config(command=self.output_text.yview)
        self.output_text.config(yscrollcommand=output_sb.set)
    ...
    def start_test(self):
        global point_set
        id_action = {}
        point_test_tree = self.input_text.get(1.0, END).split('\n')
...
#             next(point_test_tree, None)
    ...
    def out(self):
        print("正在保存测试结果......")
        self.output_text.delete(1.0, 1000.0)
        global point_set,eventKey_test_status
        for eventKey in eventKey_test_status:
            self.output_text.insert(END, eventKey + '\n')
            for status in eventKey_test_status[eventKey]:
                self.output_text.insert(END, "\t\t" + status + '\n')
            self.output_text.insert(END, "\n")
        
        self.output_text.insert(END, "本次测试的埋点是:\n")
        for point in point_set:
            self.output_text.insert(END, point + '\n')
