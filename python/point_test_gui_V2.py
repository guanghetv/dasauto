...
def atom_point_test( node):
    try:
        curTime = mktime(datetime.strptime(wd.device_time, "%a %b %d %H:%M:%S %Z %Y").timetuple())*1000
...
def test_point():
    ...
    input_frame.pack()
#         input_text = Text()
    input_sb = Scrollbar(input_frame)
    input_sb.config(command=input_text.yview)
    input_text.config(yscrollcommand=input_sb.set)
    input_sb.pack(side=RIGHT, fill=Y)
    input_text.pack(expand=YES, fill=BOTH)
    
    console_frame = Frame(master)
    console_frame.pack()
    start_btn = Button(console_frame,text="开始",command=start_test)
    stop_btn = Button(console_frame,text="退出",command=stop_test(master))
    ...
    output_frame.pack()
#         output_text = Text()
    output_sb = Scrollbar(output_frame)
    output_sb.config(command=output_text.yview)
    output_text.config(yscrollcommand=output_sb.set)
    output_sb.pack(side=RIGHT, fill=Y)
    output_text.pack(expand=YES, fill=BOTH)

    master.mainloop()


def stop_test(master):
    master.quit()
    wd.quit()
...
def start_test():
    global point_set
    id_action = {}
    point_test_tree = input_text.get(1.0, END).split('\n')
...
def out():
    print("正在保存测试结果......")
    output_text.delete(1.0, 1000.0)
    global point_set,eventKey_test_status
    for eventKey in eventKey_test_status:
        output_text.insert(END, eventKey + '\n')
        for status in eventKey_test_status[eventKey]:
            output_text.insert(END, "\t\t" + status + '\n')
        output_text.insert(END, "\n")
    
    output_text.insert(END, "本次测试的埋点是:\n")
    for point in point_set:
        output_text.insert(END, point + '\n')
        
    eventKey_test_status.clear()
    point_set.clear()
...
if __name__ == '__main__': 
    wd.start_activity("com.yangcong345.android.phone", ".presentation.activity.SplashActivity")
    print 'ffff'
    sleep(5)
#     test_point()
    master = Tk()
